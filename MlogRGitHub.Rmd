...
```{r}
newdata1 <- expand.grid(age = seq(from = 45, to = 45, by = 10), totChol = seq(from = 400, to = 400, by = 10), diabetes = c('nodm', 'dm'), gender = c('male', 'female'), cigsPerDay = seq(from = 5, to = 60), prevalentHyp = c('hpt', 'no hpt'), BMI = seq(from = 16.00, to = 60), BPMeds = c('not on hypertensive medication', 'on hypertensive medication'), TenYearCHD = c('no chd', 'chd'))
newdata1
head(newdata1)
```
...
model.mlr.new
predict.odds <- predict(model.mlr.new, newdata = newdata1, type.predict = 'link')
head(predict.odds)
```

```{r}
predict.prob <- predict(model.mlr.new, newdata = newdata1, type.predict = 'response')
head(predict.prob)
```
## Model Checking 
